...
class VibesCommand extends Command
{
    ...
    public function handle(SpotifyDiscoveryService $discovery): int
    {
        ...
        if ($this->ensureConfigured()) {
            $trackMeta = spin(
                fn (): array => $discovery->getTracks($trackIds),
        ...
        if ($missingIds !== []) {
            $oembedMeta = spin(
                fn (): array => $discovery->getTracksViaOEmbed($missingIds),
        ...
        if ($this->option('playlist')) {
            $playlistUrl = spin(
                fn (): ?string => $this->syncPlaylist($discovery, $grouped),
    ...
    private function syncPlaylist(SpotifyDiscoveryService $discovery, array $groups): ?string
    {
        ...
        $playlist = $discovery->findPlaylistByName($playlistName);

        if (! $playlist) {
            $playlist = $discovery->createPlaylist($playlistName, $description);
            if (! $playlist) {
            ...
            $discovery->updatePlaylistDetails($playlist['id'], [
        ...
        if ($uris !== []) {
            $discovery->replacePlaylistTracks($playlist['id'], $uris);
        }
